...
| **Session Log** | Notion（Wikiページ） | 作業状況 — 何が完了し、何が未処理か | **動的** |
...
3層アーキテクチャは**静的な知識**をきれいに整理してくれます。
しかし、Claude AI（アプリ）はセッションが変わると「前回何をしたか」を忘れてしまう問題がありました。

**セッションログ**は、この問題を**動的記憶**レイヤーの追加で解決します。
...
2. **「Wikiに変換」する**（必須 — 通常ページではインテグレーション接続メニューが表示されません）
3. 「接続」からインテグレーションを追加
4. [`templates/session-log-template.md`](templates/session-log-template.md) のテンプレートをコピー
5. Claude AI（アプリ）に「このページをセッションログとして使って」と伝える
...
> 💡 **なぜ「Wikiに変換」？** Notionの通常ページには「接続」メニューが表示されません。Wikiに変換することでインテグレーションからアクセスできるようになります。構築中に発見した、ちょっとしたコツです。
